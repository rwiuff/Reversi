...
\item Derfor havde vi tænkt os at få Timeren ind ved at tilføje en boolean som sætter \mintinline{java}|SpeedMode=false;| og så begyndte vi at tilføje \mintinline{java}|if (speedMode == true){} else {}| til metoderne \mintinline{java}|in(),restart(),onPaneClicked(),beginGame| og så satte vi \mintinline{java}|speedReversi(ActionEvent event){speedMode = true;|.
...
\item Løsning: 
implementere følgende i \mintinline{java}|speedReversi(ActionEvent event)|:
\item kald metoderne fra \mintinline{java}|begingame()| med følgende tilføjelser 
\mintinline{java}|controller.setSpeedMode(true); controller.getReversiTimerpane().setVisible(true);| sæt følgende metoder ind i ``begingame'' \mintinline{java}|controller.setSpeedMode(false); controller.getReversiTimerpane().setVisible(false);|
